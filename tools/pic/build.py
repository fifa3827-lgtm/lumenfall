...
def split(poly,k,rng):
    ...
    return out
def comps(g):
    return [g] if g.geom_type=='Polygon' else sorted([x for x in g.geoms if x.area>0.002],key=lambda x:-x.area)
def cut(parts,mult):
...
for fn in DESIGNS:
    ...
    for key,mult in (('base',1),('fine',1.7)):
        regs,adj,outlines,pals=cut(parts,mult)
        if key=='base':
