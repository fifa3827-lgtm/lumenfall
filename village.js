...
function vRose(cx,cy,r0,r1,n){ // 성당 둥근 창: 부채꼴 n개
 ...
 return out}
const VILLAGE=[
{set:'정원',name:'온실',img:{k:'greenhouse',x:44,y:152,h:64},hx:40,hy:125,body:`
...
{set:'집과 학교',name:'학교',img:{k:'school',x:120,y:152,h:74},hx:113,hy:118,body:`
...
{set:'축제와 계절',name:'성당',img:{k:'church',x:200,y:152,h:98},hx:200,hy:108,body:`
...
{set:'탈것',name:'기차역',img:{k:'station',x:282,y:152,h:62},hx:290,hy:124,body:`
...
{set:'하늘과 우주',name:'천문대',img:{k:'observatory',x:360,y:150,h:72},hx:365,hy:128,body:`
...
{set:'동물 친구',name:'헛간',img:{k:'barn',x:54,y:204,h:64},hx:47,hy:172,body:`
...
{set:'맛있는 것',name:'빵집',img:{k:'bakery',x:144,y:204,h:62},hx:126,hy:172,body:`
...
{set:'과일과 채소',name:'과일 가게',img:{k:'fruit',x:256,y:204,h:60},hx:274,hy:174,body:`
...
{set:'바다',name:'등대',img:{k:'lighthouse',x:330,y:206,h:92},tx:-26,hx:354,hy:150,body:`
...
function villageSVG(have){
 ...
 VILLAGE.forEach((b,bi)=>{const [g,n]=have[b.set]||[0,1],full=g>=n&&n>0;if(full)lit++;
  ...
  if(typeof ART!=='undefined'&&b.img&&ART.house[b.img.k]){
   /* 그림 건물: 불 끈 그림 위에 불 켠 그림을 모은 만큼 겹친다. 다 모으면 환하게 */
   const I=b.img,w=I.h*1.4,x=I.x-w/2,y=I.y-I.h,op=full?1:(g/n)*.75;
   const im=(src,o)=>`<image href="${src}" x="${x}" y="${y}" width="${w}" height="${I.h}" preserveAspectRatio="xMidYMax meet"${o<1?` opacity="${o.toFixed(2)}"`:''}/>`;
   body+=`<g class="vb vimg${full?' full':''}${g?'':' off'}" data-i="${bi}">`+(full?`<ellipse cx="${I.x}" cy="${I.y-I.h*.45}" rx="${I.h*.75}" ry="${I.h*.6}" fill="url(#vhalo)"/>`:'')+
    (full?'':im(ART.house[b.img.k+'_off'],1))+(op>0?im(ART.house[b.img.k],op):'')+`</g>`;return}
